...
def test_upload_builds_path(monkeypatch, tmp_path):
    ...
    expected = "abfss://climate-data-analysis@fakeacct.dfs.core.windows.net/climate_data/2020/CR20200101.parquet"
    assert cloud_uri == expected
...
def test_upload_failure(monkeypatch, tmp_path):
    ...
    p = tmp_path/"2020"/"CR20200101.parquet"
    p.parent.mkdir(parents=True); p.write_bytes(b"x")
    assert ds.upload(str(p)) is None
